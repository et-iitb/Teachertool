(function () {
  ...
  ready(function () {
    ...
    window.addEventListener("scroll", function () {
      document.querySelectorAll(".et-navbar--sticky").forEach(function (el) {
        var method = window.scrollTop > 10 ? "add" : "remove";
        el.classList[method]("et-navbar--stuck");
        if (!el.classList.contains(".et-navbar--open"))
          el.classList[method]("et-navbar--short");
      });
    });
    ...
    if (isMobile() && navigator.userAgent.match(/Chrome/i)) {
      ...
      (function (width, height) {
        ...
        window.smartresize(function () {
          ...
          var el = document.querySelector(".et-section--full-height");
          el.style.height = windowHeight + "px";
        });
      })(window.innerWidth, window.innerHeight);
    } else if (!isSupportViewportUnits) {
      // fallback for .et-section--full-height
      window.smartresize(function () {
        var el = document.querySelector(".et-section--full-height");
      ...
      $(document).on("add.cards", function (event) {
        ...
        if (
          document.querySelector("html").classList.contains("et-site-loaded") &&
          outerFind(event.target, ".et-section--full-height").length
        )
          window.dispatchEvent(new CustomEvent("resize"));
      });
    }

    // .et-section--16by9 (16 by 9 blocks autoheight)

    function calculate16by9(el) {
      el.style.height = (getWidth(el.parentNode) * 9) / 16 + "px";
    }
    window.addEventListener("smartresize", function () {
      document.querySelectorAll(".et-section--16by9").forEach(calculate16by9);
    });
    if (isJQuery)
      $(document).on("add.cards changeParameter.cards", function (event) {
        //document.addEventListener('add.cards changeParameter.cards', function (event) {
        var enabled = outerFind(event.target, ".et-section--16by9");
        if (enabled.length) {
    ...
    // .et-parallax-background

    // plugin needs replacement: Parallax

    function initParallax(card) {
      setTimeout(function () {
        outerFind(card, ".et-parallax-background").forEach(function (el) {
          if (jarallax) {
            jarallax(el, {
              speed: 0.6,
            });
    ...
    if (typeof jarallax !== "undefined" && !isMobile()) {
      window.addEventListener("update.parallax", function (event) {
        setTimeout(function () {
          if (!jarallax) return;
          var jarallax = document.querySelector(".et-parallax-background");
    ...
    if (isJQuery)
      $(document).on("add.cards delete.cards", function (event) {
        ...
        fixedTopTimeout = setTimeout(function () {
          ...
          var elm = document.querySelector(".et-fixed-top");
          if (elm) {
            fixedTop = {
    ...
    window.smartresize(function () {
      document.querySelectorAll(".et-embedded-video").forEach(function (el) {
        el.style.height =
          (
    ...
    function videoParser(card) {
      outerFind(card, "[data-bg-video]").forEach(function (el) {
        ...
        var img =
          el.querySelector(".et-background-video-preview") ||
          document.createElement("div");

        img.classList.add("et-background-video-preview");
        img.style.display = "none";
        img.style.backgroundSize = "cover";
        img.style.backgroundPosition = "center";

        if (!el.querySelector(".et-background-video-preview"))
        ...
        if (
          ...
          if (parsedUrl && /youtu\.?be/g.test(parsedUrl[3])) {
            ...
            image.onload = function () {
              ...
              if (el.querySelector(".et-background-video"))
                el.querySelector(".et-background-video").remove();

              var videoElement = document.createElement("div");

              videoElement.classList.add("et-background-video");
    ...
    document.querySelector("html").classList.add("et-site-loaded");
    window.dispatchEvent(new CustomEvent("resize"));
            ...
            if (target.hash) {
              ...
              document
                ...
                .forEach(function (el) {
                  ...
                  // var stickyMenuHeight = $('.et-navbar--sticky').length ? 64 : 0;
    ...
    document
      .querySelectorAll(".cols-same-height .et-figure")
      .forEach(function (el) {
  ...
  if (!isBuilder) {
    ...
    if (isJQuery && $.fn.socialLikes) {
      $(document).on("add.cards", function (event) {
        // document.addEventListener('add.cards', function (event) {
        outerFind(event.target, ".et-social-likes").forEach(function (el) {
          el.addEventListener(
    ...
    if (isJQuery)
      $(document).on("add.cards", function (event) {
        // document.addEventListener('add.cards', function (event) {
        if (event.target.classList.contains("et-reveal")) {
      ...
      if (isMobile()) {
        ...
        var animatedElements = Array.from(
          document.querySelectorAll(
            "p, h1, h2, h3, h4, h5, a, button, small, img, li, blockquote, .et-author-name, em, label, input, select, textarea, .input-group, .form-control, .iconbox, .btn-social, .et-figure, .et-map, .et-testimonial .card-block, .et-price-value, .et-price-figure, .dataTable, .dataTables_info"
          )
        );
            ...
            !el.parents().filter(function (parentElement) {
              if (
                parentElement.matches(
                  "a, p, .navbar, .et-arrow, footer, .iconbox, .et-slider, .et-gallery, .et-testimonial .card-block, #cookiesdirective, .et-wowslider, .accordion, .tab-content, .engine, #scrollToTop"
                )
              ) {
                return true;
              }
  ...
  ready(function () {
    if (document.querySelectorAll(".et-arrow-up").length) {
  ...
  if (!isBuilder) {
    var arrowDown = document.querySelector(".et-arrow");
    if (arrowDown) {
  ...
  if (document.querySelectorAll("nav.navbar").length) {
    ...
    if (document.querySelector(".et-after-navbar.et-fullscreen"))
      document.querySelector(
        ".et-after-navbar.et-fullscreen"
  ...
  if (!isBuilder && isIE()) {
    $(document).on("add.cards", function (event) {
      ...
      if (eventTarget.classList.contains("et-fullscreen")) {
      ...
      if (
        eventTarget.classList.contains("et-slider") ||
        eventTarget.classList.contains("et-gallery")
      ) {
        eventTarget
        ...
        if (eventTarget.classList.contains("et-slider")) {
          eventTarget
  ...
  if (!isBuilder) {
          ...
          el.addEventListener("click", function () {
            var id = el.closest(".accordionStyles").getAttribute("id"),
              ...
              sign = el.querySelector("span.sign")
                ...
                : el.querySelector("span.et-iconfont");

            if (iscollapsing.classList.contains("collapsing")) {
              if (id.indexOf("toggle") != -1 || id.indexOf("accordion") != -1) {
                if (el.classList.contains("collapsed")) {
                  sign.classList.remove("eti-arrow-up");
                  sign.classList.add("eti-arrow-down");
                } else {
                  sign.classList.remove("eti-arrow-down");
                  sign.classList.add("eti-arrow-up");
                }
                  ...
                  Array.from(accordion.children)
                    ...
                    .forEach(function (el) {
                      var icon = el.querySelector("span.sign")
                        ? el.querySelector("span.sign")
                        : el.querySelector("span.et-iconfont");
                      icon.classList.remove("eti-arrow-up");
                      icon.classList.add("eti-arrow-down");
                    });
                }
              }
            }
    ...
    // Fix for slider bug
    if (document.querySelectorAll(".et-slider.carousel").length != 0) {
      document.querySelectorAll(".et-slider.carousel").forEach(function (el) {
        var slider = el,
